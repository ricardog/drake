...
- [ ] **True Kernel Density Estimation (KDE):** Replace the current histogram-based approximation with a proper Gaussian KDE for violin plots.
- [ ] **Composite Layout Engine:** Implement a way to render the grid of plots returned by `drake-facet` into a single visual layout.
- [ ] **Gnuplot Violin Support:** Implement violin plots in the `drake-gnuplot.el` backend.
...
- [ ] **Smoothing & Uncertainty:**
    ...
    - [x] **Confidence Intervals:** Calculate and visualize the 95% confidence interval (shaded area) for the regression line. (Implemented for SVG backend)
